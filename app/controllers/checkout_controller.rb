class CheckoutController < ApplicationController
  ...
  def success
    ...
    begin
      ...
      if @session.payment_status == "paid" && @payment_type == "subscription" && @plan_type.present? && @session.subscription.present?
  ...
  def process_subscription_payment(plan_type)
    ...
    stripe_price_id = if plan_type == "trial"
                        Subscription.stripe_price_id_for(Subscription::POST_TRIAL_PLAN)
                      else
                        Subscription.stripe_price_id_for(plan_type)
                      end

    unless stripe_price_id.present?
      redirect_to plans_path, alert: "Stripe Price ID（#{Subscription.stripe_price_env_key(plan_type)}）が未設定です。"
      return
    end
