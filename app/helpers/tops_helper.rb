module TopsHelper
  ...
  FAQ_ITEMS = [
    ...
    # pricing
    { category: "pricing", q: "無料トライアルの条件は？", a: "#{Subscription::TRIAL_DAYS}日間・0円です。上限は親記事#{Subscription::PLANS[:trial][:pillar_articles]}・子記事#{Subscription::PLANS[:trial][:child_articles]}・画像#{Subscription::PLANS[:trial][:image_generations]}・タイトル提案#{Subscription::PLANS[:trial][:title_suggestions]}回（1回あたり最大#{Subscription::PLANS[:trial][:title_suggestion_max_per_use]}件）。アカウント登録だけで開始でき、クレジットカードは不要です。期間終了後は自動課金されず期限切れとなり、スタンダード（月額49,800円）への移行をご案内します。" },
    { category: "pricing", q: "料金プランと記事上限は？", a: "スタンダード月額49,800円（親#{Subscription::PLANS[:standard][:pillar_articles]}・子#{Subscription::PLANS[:standard][:child_articles]}・タイトル提案#{Subscription::PLANS[:standard][:title_suggestions]}回／1回あたり最大#{Subscription::PLANS[:standard][:title_suggestion_max_per_use]}件）／ビジネス98,000円（親#{Subscription::PLANS[:business][:pillar_articles]}・子#{Subscription::PLANS[:business][:child_articles]}・タイトル提案#{Subscription::PLANS[:business][:title_suggestions]}回／1回あたり最大#{Subscription::PLANS[:business][:title_suggestion_max_per_use]}件、AI自律あり）／エンタープライズ198,000円（親#{Subscription::PLANS[:enterprise][:pillar_articles]}・子#{Subscription::PLANS[:enterprise][:child_articles]}・タイトル提案#{Subscription::PLANS[:enterprise][:title_suggestions]}回／1回あたり最大#{Subscription::PLANS[:enterprise][:title_suggestion_max_per_use]}件）。スタンダードは初回3ヶ月15%OFF。詳細は料金セクションをご確認ください。" },
  ...
  FAQ_ITEMS_EN = [
    ...
    # pricing
    { category: "pricing", q: "What are the free trial terms?", a: "#{Subscription::TRIAL_DAYS} days at ¥0: #{Subscription::PLANS[:trial][:pillar_articles]} parent article, #{Subscription::PLANS[:trial][:child_articles]} child articles, #{Subscription::PLANS[:trial][:image_generations]} images, #{Subscription::PLANS[:trial][:title_suggestions]} title suggestions (up to #{Subscription::PLANS[:trial][:title_suggestion_max_per_use]} per use). No card needed to start. When the trial ends there is no auto-charge; you are guided to Standard (¥49,800/month)." },
    { category: "pricing", q: "What are the plans and article limits?", a: "Standard ¥49,800/mo (#{Subscription::PLANS[:standard][:pillar_articles]} parent / #{Subscription::PLANS[:standard][:child_articles]} child / #{Subscription::PLANS[:standard][:title_suggestions]} title suggestions, up to #{Subscription::PLANS[:standard][:title_suggestion_max_per_use]} per use) / Business ¥98,000 (#{Subscription::PLANS[:business][:pillar_articles]} / #{Subscription::PLANS[:business][:child_articles]} / #{Subscription::PLANS[:business][:title_suggestions]} title suggestions, up to #{Subscription::PLANS[:business][:title_suggestion_max_per_use]} per use, AI autonomous) / Enterprise ¥198,000 (#{Subscription::PLANS[:enterprise][:pillar_articles]} / #{Subscription::PLANS[:enterprise][:child_articles]} / #{Subscription::PLANS[:enterprise][:title_suggestions]} title suggestions, up to #{Subscription::PLANS[:enterprise][:title_suggestion_max_per_use]} per use). Standard includes 15% off for the first 3 months. See the pricing section." },
