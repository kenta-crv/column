class AutonomousContentRun < ApplicationRecord
  ...
  def self.start!(client:, title:, genre:, cluster_limit:, pause_for_approval_at: nil, notify_on: nil)
    settings = client.autonomous_settings_with_defaults
  ...
  def start_pillar_generation!
    with_lock do
      ...
      unless client.can_create_pillar?
      ...
      GenerateColumnBodyJob.perform_later(pillar.id, autonomous_run_id: id)
    end
  rescue => e
  ...
  def generate_child_titles!
    ...
    remaining = client.plan_limits[:child_articles] - client.child_usage_count
    if remaining <= 0
    ...
    if pause_for_child_titles?
      ...
      AutonomousContentMailer.child_titles_ready(self).deliver_later
    else
  ...
  def enqueue_next_child!
    ...
    GenerateColumnBodyJob.perform_later(next_child.id, autonomous_run_id: id)
  end
  ...
  def finalize!
    ...
    AutonomousContentMailer.cycle_complete(self).deliver_later if notify_on_list.include?("cycle_complete")
  end
  ...
  def mark_paused!(message)
    ...
    AutonomousContentMailer.limit_paused(self).deliver_later
  end
  ...
  def retry_pillar_generation!
    ...
    GenerateColumnBodyJob.perform_later(pillar_column_id, autonomous_run_id: id)
    true
  end
  ...
  def build_next_pillar_suggestions
    ...
    return [] unless client.can_suggest_titles?
    ...
    result = PillarTitleSuggestionService.call(
      ...
      target_layer: "middle",
      genre: genre,
      suggestion_count: client.max_title_suggestion_count,
      client: client
    )
    ...
    result[:titles].first(client.max_title_suggestion_count)
  rescue => e
