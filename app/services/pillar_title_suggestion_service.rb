...
class PillarTitleSuggestionService
  ...
  # ==========================================================
  def self.call(keyword1:, keyword2:, target_layer:, genre:)
    ...
    # ジャンル情報の取得
    genre_key = GenreRegistry.from_ja(genre) rescue genre
    service_info = GenreRegistry.service_profile(genre_key)
    ...
    prompt = <<~PROMPT
      ...
      与えられたキーワードとターゲット層に基づいて、検索エンジンとユーザーの双方から高く評価される「親記事（ピラーページ）タイトル案」を8個から12個の間で生成してください。
      ...
      3. 業種カテゴリとの整合性:
         必ず「#{genre}」のドメインに関連した範囲内で、かつ業種の専門性を活かしたタイトルにしてください。
      ...
      5. 多様性の確保:
         8個から12個のタイトル案について、以下の異なるアプローチをバランスよく含めてください：
         - 包括的なガイド系タイトル
         - 具体的な解決策系タイトル
         - 比較・選定系タイトル
         - トラブル解決系タイトル
    ...
    begin
      ...
      { success: true, titles: titles.map { |t| t["title"] } }
    rescue => e
