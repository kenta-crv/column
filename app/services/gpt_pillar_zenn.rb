...
# ==========================================================
# 【Zenn向けアレンジ版】gpt_pillar_generator.rb のZenn用調整版。
# ベースはQiita向け調整版(バグ修正2回分込み)。クラス名・公開メソッド名は
# 既存と同一なので、呼び出し側のコード(GenerateColumnBodyJob等)は
# 変更不要。
#
# Qiita版との主な違い(2026-08 Zenn版):
#
# 1. 【フロントマターの生成】
#    ZennはQiitaと異なり、記事のtitle/絵文字/type(tech or idea)/
#    topics/公開状態をMarkdownファイル先頭のYAMLフロントマターで
#    管理する(Zenn CLI/GitHub連携での配信を想定)。本文とは別に
#    フロントマターを組み立て、生成後の本文冒頭に付与するようにした。
#    published は常に false にし、人によるレビュー完了後に
#    別工程で true に切り替える運用を前提にしている
#    (Qiita版のレビュー待ちフローと同じ考え方)。
#
# 2. 【emoji・type(tech/idea)の生成】
#    Zennの記事にはアイキャッチ代わりの絵文字1文字と、記事種別
#    (tech=技術的な解説・手順、idea=経験談・考察寄り)が必須。
#    generate_meta_infoの出力にemoji/typeを追加し、値が不正
#    (絵文字でない、tech/idea以外)な場合は機械的にフォールバックする
#    検証ロジックを設けた。
#
# 3. 【topicsの正規化】
#    Qiita版のkeyword(カンマ区切りのタグ文言)をそのまま流用しつつ、
#    Zennのtopics配列の書式(空白なし・YAML配列)に正規化する
#    normalize_zenn_topicsを追加した。
#
# 4. 【Zenn独自のMarkdown拡張記法を許可】
#    Zennは:::message(注意書き)や:::details(折りたたみ)といった
#    独自コンテナ記法をサポートしている。本文プロンプトに、これらを
#    「使いすぎない前提で、注意点・補足情報に限って使ってよい」という
#    ガイドを追加した(Qiitaにはこの記法がないため、Qiita版では
#    触れていなかった要素)。
#
# 5. 【本文末尾の参考リンクをZennのmessageコンテナで表示】
#    Qiita版では「---」+Markdownリンクの軽いフッターにしていたが、
#    Zennでは:::messageコンテナで囲むことで、本文の地の文とも、
#    警告然とした強い表現とも違う「補足情報」らしい見た目にできる。
#
# 6. 【Qiita版までのバグ修正はすべて引き継ぎ】
#    サービス名言及の動的上限管理、まとめへのoverused_block受け渡し、
...
class GptPillarZenn
  ...
  # article_type のデフォルト値。Zenn向けのため汎用語に変更。
  # (これはcolumn側の内部管理用フィールドであり、後述のZenn記事の
  # type: tech/idea とは別概念なので、混同しないようにキー名を分けている)
  DEFAULT_ARTICLE_TYPE = "zenn_article"

  # Zennのフロントマターに書く emoji が取得できなかった/不正だった場合の
  # フォールバック値。絵文字1文字であることを厳密に検証するのは大変なので、
  # 「基本的なASCII文字だけで構成されている(=絵文字ではなさそう)」場合に
  # 限定してフォールバックする簡易チェックにしている。
  ZENN_DEFAULT_EMOJI = "📝"

  # Zennの記事種別。tech=技術的な解説・手順、idea=経験談・考察寄り。
  # generate_meta_infoの出力がこれ以外の値だった場合はtechにフォールバックする。
  ZENN_VALID_TYPES = %w[tech idea].freeze

  # Zennのtopics(タグ)は運用上5個までが実用的なので上限を設ける。
  ZENN_MAX_TOPICS = 5
  ...
  def self.generate_full_from_existing_column_impl!(column)
    ...
    puts "▶ 統合生成開始(Zenn版): #{column.title} (判定: #{target_category}, genre: #{current_genre})"
    ...
    structure_data["structure"].each_with_index do |section, idx|
      ...
      # 実例で「WordPress連携と〜」等のセクション内に##サブ見出しが
      # 複数混入し、5見出しのはずが実質15見出しになっていた事象への対処
    ...
    #
    # Qiita版では「---」+Markdownリンクの軽いフッターにしていたが、
    # Zennでは:::messageコンテナ(Zenn独自の注意書き風ブロック)で囲むことで、
    # 地の文の続きにも見えず、かといって警告のように強すぎもしない
    # 「補足情報」らしい見た目にできる。
    body_content += "\n\n:::message\n参考: [#{own_service_name}](#{own_service_url})\n:::"

    # Zennのフロントマター(title/emoji/type/topics/published)を本文冒頭に付与する。
    # Qiita版には無かった工程で、Zenn CLI/GitHub連携での配信を想定している。
    topics = normalize_zenn_topics(meta_data["keyword"])
    frontmatter = build_zenn_frontmatter(column, meta_data, topics)
    body_content = "#{frontmatter}\n#{body_content}"

    ensure_not_cancelled!(column)
    column.update!(body: body_content, status: "completed")

    puts "✅ 生成完了(Zenn版): #{clean_code}"
  ...
  def self.build_essay_context(column, genre_data, sub_data)
    ...
    contexts << <<~TEXT
      ...
      - Zenn独自のコンテナ記法(:::message ... ::: で注意書き、
        :::details タイトル ... ::: で折りたたみ)は使ってよいが、
        使いすぎない。1セクションにつき多くても1箇所まで、
        「本筋から少し外れる補足」「見落としやすい注意点」に限定して使う。
        本文中の主要な説明をこの記法の中に隠さないこと
      - 記事全体でH1見出し(# で始まる見出し)は使わない。
        タイトルはフロントマター側で管理するため、本文中の見出しは
        ## (H2)以下のみを使う
    TEXT
  ...
  def self.generate_meta_info(column, category, genre_data, sub_data, eeat_context, effective_prompt)
    prompt = <<~PROMPT
      ...
      - keywordは、Zennのtopicsとして自然な技術用語(言語名・フレームワーク名・
        ツール名・技術要素名など)を2〜4個、カンマ区切りで。
        マーケティング的な抽象語句(例:「効率化」「時短」単体など)は避け、
        具体的な技術名を優先すること。スペースを含む語句は避けること
        (Zennのtopicsはスペースなしの単語である必要があるため)
      - codeは英語スラッグ
      - typeは、記事の性質に応じて"tech"(具体的な技術解説・手順・実装が中心)
        か"idea"(経験談・考察・意見が中心で、再現手順よりも気づきが主)のどちらか
      - emojiは、記事の内容を象徴する絵文字を1文字だけ(絵文字以外の文字は含めない)
      - JSON以外禁止
  ...
  # ==========================================================
  # 【Zennフロントマター生成】(新設)
  # Zennはtitle/emoji/type/topics/publishedをMarkdown先頭のYAMLで
  # 管理する(Zenn CLI/GitHub連携での配信を想定)。ここではその
  # フロントマター文字列を組み立てるためのヘルパー群をまとめている。
  # ==========================================================

  # keyword(カンマ区切りの文言)を、Zennのtopics配列に使える形へ正規化する。
  # topicsはスペースを含められないため、空白を除去し、記号も落とす。
  def self.normalize_zenn_topics(keyword_csv)
    return [] if keyword_csv.blank?

    keyword_csv.split(",")
               .map(&:strip)
               .reject(&:blank?)
               .map { |k| k.gsub(/[\s　]+/, "").gsub(/[^0-9A-Za-zぁ-んァ-ヶー一-龠_]/, "") }
               .reject(&:blank?)
               .uniq
               .first(ZENN_MAX_TOPICS)
  end

  # type(tech/idea)の検証。想定外の値が返ってきた場合は"tech"にフォールバックする。
  def self.sanitize_zenn_type(type_value)
    value = type_value.to_s.strip.downcase
    ZENN_VALID_TYPES.include?(value) ? value : "tech"
  end

  # emojiの簡易検証。「絵文字が1文字だけ返ってくる」ことを厳密に保証するのは
  # 難しいため、「英数字・記号だけで構成されている(=絵文字が入っていなさそう)」
  # 場合や空の場合にだけフォールバックする、緩めのチェックにしている。
  def self.sanitize_zenn_emoji(emoji_value)
    value = emoji_value.to_s.strip
    return ZENN_DEFAULT_EMOJI if value.blank?
    return ZENN_DEFAULT_EMOJI if value.match?(/\A[\x00-\x7F]*\z/) # ASCIIのみ=絵文字が入っていない

    # 絵文字が複数個・他の文字と混ざって返ってきた場合に備え、先頭の1つだけを使う
    value.each_grapheme_cluster.first || ZENN_DEFAULT_EMOJI
  end

  # YAML文字列としてtitleを埋め込む際、ダブルクォートを含んでいると
  # フロントマターが壊れるためエスケープする。
  def self.escape_yaml_string(text)
    text.to_s.gsub('"', '\\"')
  end

  # Zennのフロントマターを組み立てる。published は常にfalseにし、
  # 人によるレビュー完了後に別工程でtrueへ切り替える運用を前提にしている。
  def self.build_zenn_frontmatter(column, meta_data, topics)
    zenn_type = sanitize_zenn_type(meta_data["type"])
    emoji = sanitize_zenn_emoji(meta_data["emoji"])
    topics_yaml = topics.map { |t| "\"#{escape_yaml_string(t)}\"" }.join(", ")

    <<~YAML
      ---
      title: "#{escape_yaml_string(column.title)}"
      emoji: "#{emoji}"
      type: "#{zenn_type}"
      topics: [#{topics_yaml}]
      published: false
  ...
  def self.generate_structure(column, category, genre_data, sub_data, eeat_context, effective_prompt)
    prompt = <<~PROMPT
      以下のテーマで、Zenn向け技術記事の見出し構成をJSON形式で生成してください。
  ...
  # モデルが##等の見出し行を挿入するケースがあったため(実例で
  # 「WordPress連携と〜」等のセクション内に複数の##サブ見出しが
  # 混入し、記事全体の見出し階層が崩れていた)、機械的に太字へ
  # 変換する安全網を用意する。情報の区切りとしての意図は残しつつ、
  # 見出し階層(目次やアウトライン)には影響しなくなる。
  #
  ...
  def self.call_gpt_api(prompt, json_mode: false)
    ...
    system_content = <<~SYSTEM
      ...
      - H1見出し(# )は使わない。タイトルはフロントマターが担うため、
        本文中の見出しは## (H2)以下のみを使う
      - :::message や :::details のようなZenn独自のコンテナ記法は、
        補足・注意点に限定して使ってよいが多用しない
